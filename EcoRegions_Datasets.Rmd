...
################## Works, but formatting is rough ########
table_test <- combined_ecoregions_c %>% 
  mutate(sample_weight = 1) %>% 
  group_by(ecoregion, accepted_name) %>% 
  add_tally(sample_weight) %>% 
  pivot_wider(names_from = ecoregion, values_from = n)


table_test2 <- combined_ecoregions_c %>% 
  mutate(sample_weight = 1) %>% 
  group_by(ecoregion, accepted_name) %>% 
  replace_na(list(x = 0)) %>% 
  pivot_wider(names_from = ecoregion, values_from = sample_weight )

combined_ecoregions_test <- combined_ecoregions_c

combined_ecoregions_test$accepted_name <- as.factor(combined_ecoregions_test$accepted_name)

table_test3 <- combined_ecoregions_test %>% 
  group_by(ecoregion) %>% 
  tally(accepted_name)
############################### This line of code makes it where we have equal sizes of species for each ecoregion. The Output is NA however when that species is not present.###########
table_test3 <- combined_ecoregions_test %>% 
...
test <- combined_ecoregions_test %>% 
  ...
  filter(accepted_name == "Andrena sperryi")
#############################
table_test2 <- combined_ecoregions_c %>% 
  tally(group_by(accepted_name))
#####################################################
check <- combined_ecoregions_c %>% 
  filter(ecoregion == "Coast_Range") %>% 
  filter(accepted_name == "Bombus occidentalis")


combined_ecoregions_c_table <- combined_ecoregions_c %>% 
  group_by(ecoregion, accepted_name) %>% 
  add_tally(accepted_name) %>% 
  mutate(species_count = n) %>% 
  select(-n)

Ecoregions_pivoted <- combined_ecoregions_c_table %>% 
  #mutate(row = row_number()) %>% 
  #separate_rows() %>% 
  #select(ecoregion, accepted_name, row) %>% 
  pivot_wider(names_from = ecoregion, values_from = species_count)

# Sum up sampling events into their own singular column
combined_ecoregions_c_table_sampling_events <- combined_ecoregions_c_table %>% 
  group_by(ecoregion, year_intervals) %>% 
  add_count(year_intervals) %>% 
  mutate(sampling_count = n) %>% 
  select(-n)

# Pivot wider
Ecoregions_pivoted <- combined_ecoregions_c_table_sampling_events %>% 
  mutate(row = row_number()) %>% 
  separate_rows() %>% 
  #select(ecoregion, accepted_name, row) %>% 
  pivot_wider(names_from = ecoregion, values_from = sampling_count)
  


#pivot_wider(names_from = ecoregion, values_from = accepted_name)

## Check
check <- combined_ecoregions_c %>% 
  filter(ecoregion == "Coast_Range") %>% 
  filter(accepted_name == "Bombus occidentalis") %>% 
  filter(year_intervals == '(1.91e+03,1.92e+03]')
```
