...
leaflet(cali_ecoregions) %>% 
  ...
  addLegend("bottomright", pal = colpal_ext, values = cali_ecoregions$NA_L3NAME)
...
Coast_Range_Bees <- bees[Coast_Range,]
Central_Basin_and_Range_Bees <- bees[Central_Basin_and_Range,]
Mojave_Basin_and_Range_Bees <- bees[Mojave_Basin_and_Range,]
Cascades_Bees <- bees[Cascades,]
Sierra_Nevadas_Bees <- bees[Sierra_Nevadas,]
Cali_Coastal_Sage_Chap_Oak_Woodlands_Bees <- bees[Cali_Coastal_Sage_Chap_Oak_Woodlands,]
Central_Cali_Valley_Bees <- bees[Central_Cali_Valley,]
Klamath_Mountains_Bees <- bees[Klamath_Mountains,]
Southern_and_Baja_Cali_PineOak_Mounts_Bees <- bees[Southern_and_Baja_Cali_PineOak_Mounts,]
Northern_Basin_and_Range_Bees <- bees[Northern_Basin_and_Range,]
Sonoran_Desert_Bees <- bees[Sonoran_Desert,]
Eastern_Cascades_Slopes_and_Foothills_Bees <- bees[Eastern_Cascades_Slopes_and_Foothills,]
...
# Drop anything that does not contain a year 
...
# And bring them back into one dataframe
...
# Estimated Richness of each ecoregion with the names
...
ggplot(data = ecoregion_w_parms, aes(x = lowest_mean_temp, y = Estimator)) + 
  geom_point() +
  #stat_smooth(method = "lm") +
  theme_bw() +
  xlab("Mean Lowest Temperature") +
  ylab("Estimated Species Richness") + 
  ggtitle("Bee Species Richness Correlation with Mean Lowest Temperature") +
  theme(plot.title = element_text(hjust = 0.5))
...
shapiro.test(ecoregion_w_parms$mean_percipitation_mm) # Does not PASS

qqPlot(ecoregion_w_parms$highest_mean_temp, add.line = TRUE)

qqPlot(ecoregion_w_parms$Estimator, add.line = TRUE)

qqPlot(ecoregion_w_parms$mean_percipitation_mm, add.line = TRUE)
