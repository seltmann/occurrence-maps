...
ggiNEXT(regions_i, type=3, se=TRUE, facet.var="site", color.var="site", grey=FALSE)

ggiNEXT(regions_i, type=1, se=TRUE, facet.var="site", color.var="site", grey=FALSE)

```
```{r}
view_test <- rbind(MBRB_drop, SNB_drop, CSHOB_drop)

view_mat_set <-view_test %>% group_by(ecoregion, accepted_name) %>% 
                          summarize(n=n()) %>% spread(accepted_name, n, fill=0)


view_mat <-as.data.frame(view_mat_set)[,-1]

# Reformat with bins as the columns and species as the rows for estimating coverage based rarefaction 
view_df = data.frame(t(view_mat))

colnames(view_df) <- c("Chaparral", "Mojave", "Sierra Nevadas")

view_i <- iNEXT(view_df, q=0, datatype="abundance")

print(view_i)

ggiNEXT(view_i, type=1, se=TRUE, facet.var="none", color.var="site", grey=FALSE)
```
