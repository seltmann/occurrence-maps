...
ggplot() +
  ...
  geom_density2d(data = data_hymenpotera, aes(x = lon, y = lat)) +
                 stat_density2d(data = data_hymenpotera, aes(x = lon, y = lat, fill = ..level..), contour_var = "density", contour = TRUE, adjust =  c(.0045, .0068))
...
```
## Using the package tmaps, lets visualize the data a bit differently. 
...
tm_shape(copr_boundary_2020) +
  tm_borders() +
   tm_graticules() +
  tm_shape(data_isopoda) + tm_bubbles(size = "count", col = "red") +
   tm_compass(position = c("left", "top")) +
