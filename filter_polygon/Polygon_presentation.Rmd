...
ggplot() +
  ...
  ggtitle("Organismal Occurrences In Santa Barbara County") +
  theme_bw() +
...
ggplot() +
  ...
  ggtitle("Arthropod Occurrences within Coal Oil Point") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.title = element_text( hjust = 0.5))
...
```
### Lets check how many unique represenatives there are for this bounded data
```{r}
unique(occur_sf_order_new_subset$order) #There is 17 unique orders in of arthropods in this bounded region
...
### This section is annotated out since I dont want to run in multiple times. 
...
```
### Now that we've made unique columns for the lat and lon, lets count up the number of observations per order in each location. 
```{r}
...
occurrence_order_test <- occurence_order_subset_sep %>% 
  group_by(lat, lon) %>% 
  summarise(n = n()) %>% 
  arrange(desc(n))
occurrence_order_test #Same output as before, just a double check I found online

occurence_order_subset_sep_unique %>% 
  group_by(lat, lon, order) %>% 
  summarize(sum(count))

occurrence_order_subset_sep_unique %>% 
  count(order)

unique(occurrence_order_subset_sep_unique$Order)
```
...
```{r}
filter_taxa <- function(data, kingdom = NULL, phylum = NULL, class = NULL, order = NULL, family = NULL, genus = NULL, specificEpithet = NULL) { #create a function
  if(!is.null(kingdom)){
  ...
  if(!is.null(family)){
    family_output <- data[data$family %in% family,]
  }
  if(!is.null(genus)){
    genus_output <- data[data$genus %in% genus,]
  }
  if(!is.null(specificEpithet)){
    specificEpithet_output <- data[data$specificEpithet %in% specificEpithet,]
  }
  ...
  df <- as.data.frame(kingdom_output, phylum_output, class_output, order_output, family_output, genus_output, specificEpithet_output)
  return(df)
}

test_taxa_filter <- filter_taxa(specimen_data, kingdom = "Animalia", phylum = "Arthropoda", class = "Malacostraca", order = "Decapoda", family = "Palinuridae", genus = "Panulirus", specificEpithet = "crassipes" )
