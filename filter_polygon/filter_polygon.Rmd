...
#Prehaps we need to assign a Rasterlayer to this? 
specimen_data_less_longlat_crs <- crs("+proj=longlat +datum=NAD83 +no_defs +ellps=WGS84 +towgs84=0,0,0")

crs(specimen_data_less_longlat_crs)

plot_locations_specimen <- st_as_sf(specimen_data_less_longlat_crs, coords = c("decimalLongitude", "decimalLatitude"), crs = longlat)
```



```{r}
#Since the species data is in degrees decimal format, reprojecting it to standard Lat/Long may be the answer
specimen_data_less_reproj <- projectRaster(specimen_data_less_longlat_crs, 
                                           crs = crs(copr_boundary_2020))
#Wont work, maybe other way?

copr_boundary_2020_reproj <- projectRaster(copr_boundary_2020,
                                           crs = crs(specimen_data_less_longlat_crs))
...
```{r}
#This did not work due to unspecified CRS, so maybe look into metadata and find how to specify this? Or look for an alternative method. 
ggplot() +
  geom_sf(data = copr_boundary_2020, fill = "palegreen", color = "black") +
  geom_point(data = plot_locations_specimen, aes(x = x, y = y), shape = 1)
```
