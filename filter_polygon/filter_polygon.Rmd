...
ggplot() +
  ...
  ggtitle("Organismal Occurrences within Coal Oil Point") +
  theme_bw() +
...
# This next section is designated as holding some work I've been doing in the shiny app since the saving feature in it is quite strange.

## Some inputs for the map app that work
```{r}
#creates a drop down menu to select the order for us to look at 
selectInput(inputId = "selected_order",
                        label = "select a order",
                        choices = occurrence_filtered[2])

```


## Some work in progress outputs for the mapping plot

```{r}
#This line of code gives an error where it says mapping must be defined in aes, which it clearly is so there probably is something else going on here. 
server <- function(input, output) {
    current_order <- reactive({input$selected_order})
    
    output$myplot <- renderPlot({
        ggplot() +
            geom_sf(current_order, mapping = aes(geometry = geometry)) +
            geom_sf(copr_boundary)
    })
