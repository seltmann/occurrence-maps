...
```
```{r}
trimdata(occur_cropped_all, copr_boundary_2020, rectangle = F, buffer = 0.05) 
```

```{r}
occur_cropped_trimmed <- st_crop(occur_cropped_all,copr_boundary_extent)

extent(occur_cropped_all)

extent(occur_cropped_trimmed)
```

```{r}
ggplot() +
  geom_sf(data = copr_boundary_2020, fill = "palegreen", color = "black") +
  geom_sf(occur_cropped_trimmed, mapping = aes(geometry = geometry), size = 1) +
  ggtitle("Occurence of Plant Specimens within Coal Oil Point")
```
