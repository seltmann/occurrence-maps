...
### (Optional) Here the user may select from a variety of subsetting functions in order to choose taxonomic order levels of interest. 
```{r}
unique(occurrence_data_cleaned$kingdom) #This will allow you to see all the unique names of the available kingdoms of your data
#NOTE: if your data is

occurrence_data_cleaned %>% 
  dplyr::filter(kingdom == "insert_kingdom") %>% 
  dplyr::filter(phylum == "insert_phylum") %>% 
  dplyr::filter(class == "insert_class") %>% 
  dplyr::filter(family == "insert_family") %>% 
  dplyr::filter(genus == "insert_genus") %>% 
  dplyr::filter(species == "insert_species")

unique(occurrence_data_cleaned)
