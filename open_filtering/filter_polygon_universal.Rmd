...
Author: JT Miller & Katja Seltmann
---
...
```{r}
occurrence_data_cleaned_filtered <- taxa_filter(data = occurrence_data_cleaned, kingdom = NULL, phylum = NULL, class = NULL, order = NULL, family = NULL, genus = NULL, specificEpithet = NULL)
...
occurrence_w_crs <- st_as_sf(occurrence_data_cleaned_filtered, coords = c('decimalLongitude', 'decimalLatitude'), crs = 0000) 
...
plotly_plot <- ggplot() +
                ...
                geom_sf(occurrence_bounded, mapping = aes(lon, lat, color = *insert_taxonomic_rank_here*, count = count)) +
...
ggplotly(plotly_plot, tooltip = c("*insert_taxonomic_rank_here*", "count"))
```
...
specimen_data <- read.delim(file="Insert_text_file_here",header=TRUE)
...
specimen_data_w_crs <- st_as_sf(specimen_data, coords = c('decimalLongitude', 'decimalLatitude'), crs = 0000) 
...
boundary_map <- sf::read_sf("Insert_Shapefile_Here") %>% 
  sf::st_transform('Insert_proj4_string_here')
...
```{r}
specimen_bounded_taxa <- taxa_filter(data = specimen_bounded_sep, kingdom = NULL, phylum = NULL, class = NULL, order = NULL, family = NULL, genus = NULL, specificEpithet = NULL)
#Replace the NULL with whatever your filtering to. 
```
