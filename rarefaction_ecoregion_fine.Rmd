...
library(lmtest)
library(psych)
Bombus_richness <- ChaoRichness(Bombus_df, datatype = "abundance")

Bombus_richness_df <- data.frame(Bombus_richness)

### Create a vector for lowest/highest annual temp
lowest_mean_temp <- c(-1, 2, 7, 2, 5, 19, -3, 14, 5, 6, 5, 15)
highest_mean_temp <- c(11, 11, 14, 14, 24, 25, 17, 18, 14, 17, 9, 19)
...
Bombus_w_params <- cbind(Bombus_richness_df, lowest_mean_temp, highest_mean_temp, mean_percipitation_mm)

pairs.panels(Bombus_w_params, density = TRUE, cor = FALSE, lm = TRUE)


Bombus_w_params$log_Estimator <- log(Bombus_w_params$Estimator)

Bombus_w_params$log_mean_percipitation_mm <- log(Bombus_w_params$mean_percipitation_mm)
...
plot(data = Bombus_w_params, log_Estimator ~ log_mean_percipitation_mm)

plot(data = Bombus_w_params, log_Estimator ~ highest_mean_temp)

plot(data = Bombus_w_params, log_Estimator ~ lowest_mean_temp)
